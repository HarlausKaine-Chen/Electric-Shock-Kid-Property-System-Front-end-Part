<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">
        <el-form-item class="textarea" v-if="type!='info'" label="报修内容" prop="repairText">
          <el-input
            style="min-width: 200px; max-width: 600px;"
            type="textarea"
            :rows="8"
            placeholder="报修内容"
            v-model="ruleForm.repairText" >
          </el-input>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.repairText" label="报修内容" prop="repairText">
              <span>{{ruleForm.repairText}}</span>
          </el-form-item>
        </div>
      </el-col>
...
info(id) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `repair/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
