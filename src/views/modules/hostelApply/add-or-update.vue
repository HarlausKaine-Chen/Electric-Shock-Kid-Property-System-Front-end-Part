<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
                        <el-col :span="12">
          <el-form-item class="input" label="学生姓名" prop="name">
              <el-input v-model="ruleForm.name" 
                placeholder="学生姓名" readonly></el-input>
                                    ...
                                    <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="申请描述" prop="applyTxt">
          <el-input v-model="ruleForm.applyTxt" 
              placeholder="申请描述" clearable  :readonly="ro.applyTxt"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="申请描述" prop="applyTxt">
              <el-input v-model="ruleForm.applyTxt" 
                placeholder="申请描述" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
	  <el-col :span="12">
		<el-form-item class="select" v-if="userType=='学生'"  label="申请类别" prop="applyType">
	            <el-select v-model="ruleForm.applyType" placeholder="请选择申请类别">
	              <el-option
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    if(o=='applyTxt'){
            this.ruleForm.applyTxt = obj[o];
	    this.ro.applyTxt = true;
            continue;
          }
		}
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
			var json = data.data;
			//如果this.type为空，那么是新增，需要将姓名加上
			if(this.type == '' || this.type == undefined){
				this.ruleForm.name=json.name;
				this.ruleForm.stuId = json.id;
				this.ruleForm.applyPro = '待审核';
			}
...
info(id) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `hostelApply/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
